...
class HybridSolver:
    ...
    def _precompute_embeddings(self, batch_size=100):
        ...
        # Build paths
        self.document_basenames = sorted(image_dict.keys())
        self.document_image_paths = [image_dict[bn] for bn in self.document_basenames]
        ...
        for i in range(0, len(self.document_image_paths), batch_size):
            ...
            with torch.no_grad():
                ...
                if hasattr(image_embeds_clip, 'pooler_output'):
                    image_embeds_clip = image_embeds_clip.pooler_output
                elif not isinstance(image_embeds_clip, torch.Tensor):
                    image_embeds_clip = image_embeds_clip[0]
            ...
            del images_clip, images_siglip, images_openclip, image_inputs_clip, image_inputs_siglip, image_inputs_openclip
            torch.cuda.empty_cache()
            
            print(f"Processed image batch {i//batch_size + 1}/{(len(self.document_image_paths) + batch_size - 1)//batch_size}")
        
        self.image_embeds_clip = torch.cat(all_clip_embeds, dim=0)
        ...
        if self.keep_embeddings_on_gpu:
            ...
            self.image_embeds_clip = self.image_embeds_clip.to(self.device)
    ...
    def get_combined_top_k_documents(self, needle_word, pos_image, k=10):
        """Get top-k documents using 3 multimodal models (image-only)."""
        start_time = time.time()
        ...
        with torch.no_grad():
            ...
            text_embeds_clip = self.clip_model.get_text_features(**text_inputs_clip)
            # Handle model output object - extract the actual tensor
            if hasattr(text_embeds_clip, 'pooler_output'):
                text_embeds_clip = text_embeds_clip.pooler_output
            elif not isinstance(text_embeds_clip, torch.Tensor):
                text_embeds_clip = text_embeds_clip[0]
            text_embeds_clip = text_embeds_clip.squeeze(0)
            ...
            if self.keep_embeddings_on_gpu:
                image_embeds_clip_gpu = self.image_embeds_clip
                image_embeds_siglip_gpu = self.image_embeds_siglip
                image_embeds_openclip_gpu = self.image_embeds_openclip
            else:
                image_embeds_clip_gpu = self.image_embeds_clip.to(self.device)
            ...
            similarities_clip = (image_embeds_clip_gpu @ text_embeds_clip.unsqueeze(1)).squeeze(1)
            ...
            # Combine scores - equal weight for 3 multimodal models
            combined_scores = (
                similarities_clip + 
                similarities_siglip + 
    ...
    def process_dataset(self, dataset_file, output_dir, top_k_filter=20):
        ...
        with open(log_file, 'w') as log:
            ...
            log.write(f"Average Combined Encoder (3 multimodal models) Inference Time: {avg_combined_time:.4f} seconds\n")
        ...
        print(f"Average Combined Encoder (3 multimodal models) Inference Time: {avg_combined_time:.4f} seconds")
        print(f"Average Qwen2-VL Filtering Time: {avg_qwen_filtering_time:.4f} seconds")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Image-only Retrieval with 3 multimodal models.")
